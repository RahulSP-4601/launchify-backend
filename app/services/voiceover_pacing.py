...
def fit_voice_line(text: str, available_seconds: float) -> str:
    cleaned = polish(text)
    target_seconds = max(available_seconds - 0.06, 0.95)
...
def trimmed_words(text: str, available_seconds: float) -> str:
    limit = max(MIN_WORDS, min(len(text.split()), int(round(max(available_seconds, 1.0) * 2.55))))
...
def estimated_duration(text: str) -> float:
    ...
    return round(max(1.8, min(9.5, words / 2.45)), 2)
