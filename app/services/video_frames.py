...
def extract_scene_frames(
    ...
    frames_by_scene: dict[int, list[Path]] = {}
    for scene_number, scene_range in zip(scene_numbers, scene_ranges, strict=True):
        ...
        frames_by_scene[scene_number] = [
            extract_frame(video_path, output_dir, scene_number, frame_index, frame_time)
...
def sample_times(start: float, end: float) -> list[float]:
    ...
    lead = start + min(0.35, duration * 0.2)
    tail = end - min(0.35, duration * 0.2)
    return [round(lead, 2), round(midpoint, 2), round(max(start, tail), 2)]
