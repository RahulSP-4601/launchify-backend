    ...
    def list_projects(self, user_id: str) -> list[ProjectSummary]:
        ...
        return [
            ProjectSummary(
                ...
                has_launch_script=bool(row[8]),
                has_edit_plan=bool(row[9]),
                has_quality_report=bool(row[10]),
                has_benchmark_report=bool(row[11]),
                has_voiceover=bool(row[12]),
                has_preview_video=bool(row[13]),
                has_final_video=bool(row[14]),
            )
            for row in rows
        ]
